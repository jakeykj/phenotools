...
def phenotypes_to_excel_worksheet(phenotypes, ws, highlight):
...
def phenotypes_to_xlsx(phenotypes, filepath, ws_name=None, highlight=False):
    wb = Workbook()
    ws = wb.active
    if ws_name:
        ws.title = ws_name
    phenotypes_to_excel_worksheet(phenotypes, ws, highlight)
    wb.save(filepath)
